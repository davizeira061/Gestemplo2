					...
					if ($bValidEmail) {
						...
						if (EW_ENCRYPTED_PASSWORD) {
							if (strtolower($this->Action) == "confirm") {
								...
								$rsnew = array('senha' => $sPassword); // Reset the password
								$this->Update($rsnew);
							}
							} else {
...
<?php } ?>
		...
		<div class="col-sm-10 input-group"><div class="input-group-addon"><i class="fa fa-user"></i></div>
		<input type="text" name="email" id="email" class="form-control ewControl" value="<?php ew_HtmlEncode($forgotpwd->Email) ?>" size="30" maxlength="40"></div>
		</div>
		<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
		<button class="btn btn-primary ewButton" name="btnsubmit" id="btnsubmit" type="submit"><i class="fa fa-envelope"></i>&nbsp;<?php echo $Language->Phrase("SendPwd") ?></button>
		</div>
		</div>
		</form>
		<script type="text/javascript">
