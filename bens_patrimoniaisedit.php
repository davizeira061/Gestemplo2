...
$bens_patrimoniais_edit = NULL; // Initialize page object first

class cbens_patrimoniais_edit extends cbens_patrimoniais {
	...
	// Page object name
	var $PageObjName = 'bens_patrimoniais_edit';
	...
	function Page_Init() {
		...
		$this->Id_Patri->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->Id_Patri->CurrentValue == "")
			$this->Page_Terminate("bens_patrimoniaislist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->Id_Patri->FldIsDetailKey)
			$this->Id_Patri->setFormValue($objForm->GetValue("x_Id_Patri"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->Id_Patri->CurrentValue = $this->Id_Patri->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Localidade
			$this->Localidade->SetDbValueDef($rsnew, $this->Localidade->CurrentValue, NULL, $this->Localidade->ReadOnly);

			// Descricao
			$this->Descricao->SetDbValueDef($rsnew, $this->Descricao->CurrentValue, NULL, $this->Descricao->ReadOnly);

			// DataAquisao
			$this->DataAquisao->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->DataAquisao->CurrentValue, 7), NULL, $this->DataAquisao->ReadOnly);

			// Tipo
			$this->Tipo->SetDbValueDef($rsnew, $this->Tipo->CurrentValue, NULL, $this->Tipo->ReadOnly);

			// Estado_do_bem
			$this->Estado_do_bem->SetDbValueDef($rsnew, $this->Estado_do_bem->CurrentValue, NULL, $this->Estado_do_bem->ReadOnly);

			// Valor_estimado
			$this->Valor_estimado->SetDbValueDef($rsnew, $this->Valor_estimado->CurrentValue, NULL, $this->Valor_estimado->ReadOnly);
...
// Create page object
if (!isset($bens_patrimoniais_edit)) $bens_patrimoniais_edit = new cbens_patrimoniais_edit();

// Page init
$bens_patrimoniais_edit->Page_Init();

// Page main
$bens_patrimoniais_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$bens_patrimoniais_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var bens_patrimoniais_edit = new ew_Page("bens_patrimoniais_edit");
bens_patrimoniais_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = bens_patrimoniais_edit.PageID; // For backward compatibility

// Form object
var fbens_patrimoniaisedit = new ew_Form("fbens_patrimoniaisedit");

// Validate form
fbens_patrimoniaisedit.Validate = function() {
...
fbens_patrimoniaisedit.Form_CustomValidate = 
...
fbens_patrimoniaisedit.ValidateRequired = true;
<?php } else { ?>
fbens_patrimoniaisedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fbens_patrimoniaisedit.Lists["x_Localidade"] = {"LinkField":"x_Id_igreja","Ajax":null,"AutoFill":false,"DisplayFields":["x_Igreja","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fbens_patrimoniaisedit.Lists["x_Estado_do_bem"] = {"LinkField":"x_Id_est_patri","Ajax":null,"AutoFill":false,"DisplayFields":["x_Estado_do_Bem","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $bens_patrimoniais_edit->ShowPageHeader(); ?>
<?php
$bens_patrimoniais_edit->ShowMessage();
?>
<form name="fbens_patrimoniaisedit" id="fbens_patrimoniaisedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($bens_patrimoniais_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $bens_patrimoniais_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fbens_patrimoniaisedit.Lists["x_Localidade"].Options = <?php echo (is_array($bens_patrimoniais->Localidade->EditValue)) ? ew_ArrayToJson($bens_patrimoniais->Localidade->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fbens_patrimoniaisedit", "x_DataAquisao", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fbens_patrimoniaisedit.Lists["x_Estado_do_bem"].Options = <?php echo (is_array($bens_patrimoniais->Estado_do_bem->EditValue)) ? ew_ArrayToJson($bens_patrimoniais->Estado_do_bem->EditValue, 0) : "[]" ?>;
</script>
</span>
...
<span id="el_bens_patrimoniais_Id_Patri">
<input type="hidden" data-field="x_Id_Patri" name="x_Id_Patri" id="x_Id_Patri" value="<?php echo ew_HtmlEncode($bens_patrimoniais->Id_Patri->CurrentValue) ?>">
</span>
...
<script type="text/javascript">
fbens_patrimoniaisedit.Init();
</script>
<?php
$bens_patrimoniais_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$bens_patrimoniais_edit->Page_Terminate();
?>
