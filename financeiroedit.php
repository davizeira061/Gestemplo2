...
$financeiro_edit = NULL; // Initialize page object first

class cfinanceiro_edit extends cfinanceiro {
	...
	// Page object name
	var $PageObjName = 'financeiro_edit';
	...
	function Page_Main() {
		...
		if ($this->Id->CurrentValue == "")
			$this->Page_Terminate("financeirolist.php"); // Invalid key, return to list
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Tipo_Recebimento
			$this->Tipo_Recebimento->SetDbValueDef($rsnew, $this->Tipo_Recebimento->CurrentValue, 0, $this->Tipo_Recebimento->ReadOnly);
...
// Create page object
if (!isset($financeiro_edit)) $financeiro_edit = new cfinanceiro_edit();

// Page init
$financeiro_edit->Page_Init();

// Page main
$financeiro_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$financeiro_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var financeiro_edit = new ew_Page("financeiro_edit");
financeiro_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = financeiro_edit.PageID; // For backward compatibility

// Form object
var ffinanceiroedit = new ew_Form("ffinanceiroedit");

// Validate form
ffinanceiroedit.Validate = function() {
...
ffinanceiroedit.Form_CustomValidate = 
...
ffinanceiroedit.ValidateRequired = true;
<?php } else { ?>
ffinanceiroedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ffinanceiroedit.Lists["x_FormaPagto"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Forma_Pagto","","",""],"ParentFields":["x_Tipo_Recebimento"],"FilterFields":["x_filtro_tipo_recebimento"],"Options":[]};
ffinanceiroedit.Lists["x_Conta_Caixa"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Conta_Caixa","","",""],"ParentFields":["x_Tipo"],"FilterFields":["x_Tipo"],"Options":[]};
ffinanceiroedit.Lists["x_Situacao"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Situacao","","",""],"ParentFields":["x_Tipo"],"FilterFields":["x_id_tipo"],"Options":[]};
ffinanceiroedit.Lists["x_id_discipulo"] = {"LinkField":"x_Id_membro","Ajax":null,"AutoFill":false,"DisplayFields":["x_Nome","x_CPF","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ffinanceiroedit.Lists["x_Centro_de_Custo"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Conta","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $financeiro_edit->ShowPageHeader(); ?>
<?php
$financeiro_edit->ShowMessage();
?>
<form name="ffinanceiroedit" id="ffinanceiroedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($financeiro_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $financeiro_edit->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ffinanceiroedit.Lists["x_FormaPagto"].Options = <?php echo (is_array($financeiro->FormaPagto->EditValue)) ? ew_ArrayToJson($financeiro->FormaPagto->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<?php } ?>
<script type="text/javascript">
ffinanceiroedit.Lists["x_Conta_Caixa"].Options = <?php echo (is_array($financeiro->Conta_Caixa->EditValue)) ? ew_ArrayToJson($financeiro->Conta_Caixa->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
ffinanceiroedit.Lists["x_Situacao"].Options = <?php echo (is_array($financeiro->Situacao->EditValue)) ? ew_ArrayToJson($financeiro->Situacao->EditValue, 0) : "[]" ?>;
</script>
</span>
...
<?php if ($financeiro->id_discipulo->Visible) { // id_discipulo ?>
	<div id="r_id_discipulo" class="form-group">
		<label id="elh_financeiro_id_discipulo" for="x_id_discipulo" class="col-sm-2 control-label ewLabel"><?php echo $financeiro->id_discipulo->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $financeiro->id_discipulo->CellAttributes() ?>>
<span id="el_financeiro_id_discipulo">
<select data-field="x_id_discipulo" id="x_id_discipulo" name="x_id_discipulo"<?php echo $financeiro->id_discipulo->EditAttributes() ?>>
<?php
if (is_array($financeiro->id_discipulo->EditValue)) {
	$arwrk = $financeiro->id_discipulo->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($financeiro->id_discipulo->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$financeiro->id_discipulo) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
<script type="text/javascript">
ffinanceiroedit.Lists["x_id_discipulo"].Options = <?php echo (is_array($financeiro->id_discipulo->EditValue)) ? ew_ArrayToJson($financeiro->id_discipulo->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php echo $financeiro->id_discipulo->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("ffinanceiroedit", "x_Dt_Lancamento", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("ffinanceiroedit", "x_Vencimento", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php } ?>
<script type="text/javascript">
ffinanceiroedit.Lists["x_Centro_de_Custo"].Options = <?php echo (is_array($financeiro->Centro_de_Custo->EditValue)) ? ew_ArrayToJson($financeiro->Centro_de_Custo->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<span id="el_financeiro_Id">
<input type="hidden" data-field="x_Id" name="x_Id" id="x_Id" value="<?php echo ew_HtmlEncode($financeiro->Id->CurrentValue) ?>">
</span>
...
<script type="text/javascript">
ffinanceiroedit.Init();
</script>
<?php
$financeiro_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$financeiro_edit->Page_Terminate();
?>
