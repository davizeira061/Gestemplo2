...
$dizimos_grid = NULL; // Initialize page object first

class cdizimos_grid extends cdizimos {
	...
	// Page object name
	var $PageObjName = 'dizimos_grid';

	// Grid form hidden field names
	var $FormName = 'fdizimosgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["dizimos"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function ClearInlineMode() {
		$this->Receitas->FormValue = ""; // Clear form value
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_Descricao") && $objForm->HasValue("o_Descricao") && $this->Descricao->CurrentValue <> $this->Descricao->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Receitas") && $objForm->HasValue("o_Receitas") && $this->Receitas->CurrentValue <> $this->Receitas->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_FormaPagto") && $objForm->HasValue("o_FormaPagto") && $this->FormaPagto->CurrentValue <> $this->FormaPagto->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Dt_Lancamento") && $objForm->HasValue("o_Dt_Lancamento") && $this->Dt_Lancamento->CurrentValue <> $this->Dt_Lancamento->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Vencimento") && $objForm->HasValue("o_Vencimento") && $this->Vencimento->CurrentValue <> $this->Vencimento->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function LoadFormValues() {
		...
		$this->Dt_Lancamento->setOldValue($objForm->GetValue("o_Dt_Lancamento"));
		...
		$this->Vencimento->setOldValue($objForm->GetValue("o_Vencimento"));
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->Receitas->EditValue) <> "" && is_numeric($this->Receitas->EditValue)) {
			$this->Receitas->EditValue = ew_FormatNumber($this->Receitas->EditValue, -2, -2, -2, -2);
			$this->Receitas->OldValue = $this->Receitas->EditValue;
			}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->id_discipulo->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@Id_membro@", ew_AdjustSql($this->id_discipulo->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// id_discipulo
		if ($this->id_discipulo->getSessionValue() <> "") {
			$rsnew['id_discipulo'] = $this->id_discipulo->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "membro") {
			$this->id_discipulo->Visible = FALSE;
			if ($GLOBALS["membro"]->EventCancelled) $this->EventCancelled = TRUE;
		}
