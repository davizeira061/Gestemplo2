...
//
// Table class for Agenda_Morta
//
class cAgenda_Morta extends cTable {
	var $id;
	var $Prioridade;
	var $Data;
	var $Horario;
	...
	function __construct() {
		...
		$this->TableVar = 'Agenda_Morta';
		$this->TableName = 'Agenda_Morta';
		...
		// id
		$this->id = new cField('Agenda_Morta', 'Agenda_Morta', 'x_id', 'id', 'agenda.id', 'agenda.id', 3, -1, FALSE, 'agenda.id', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// Prioridade
		$this->Prioridade = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Prioridade', 'Prioridade', 'agenda.Prioridade', 'agenda.Prioridade', 16, -1, FALSE, 'agenda.Prioridade', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Prioridade->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Prioridade'] = &$this->Prioridade;

		// Data
		$this->Data = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Data', 'Data', 'agenda.Data', 'DATE_FORMAT(agenda.Data, \'%d/%m/%Y\')', 133, 7, FALSE, 'agenda.Data', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Data->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['Data'] = &$this->Data;

		// Horario
		$this->Horario = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Horario', 'Horario', 'agenda.Horario', 'DATE_FORMAT(agenda.Horario, \'%d/%m/%Y\')', 134, -1, FALSE, 'agenda.Horario', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Horario->FldDefaultErrMsg = $Language->Phrase("IncorrectTime");
		$this->fields['Horario'] = &$this->Horario;

		// Assunto
		$this->Assunto = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Assunto', 'Assunto', 'agenda.Assunto', 'agenda.Assunto', 200, -1, FALSE, 'agenda.Assunto', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Assunto'] = &$this->Assunto;

		// Tarefa
		$this->Tarefa = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Tarefa', 'Tarefa', 'agenda.Tarefa', 'agenda.Tarefa', 201, -1, FALSE, 'agenda.Tarefa', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Tarefa'] = &$this->Tarefa;

		// Resolvido
		$this->Resolvido = new cField('Agenda_Morta', 'Agenda_Morta', 'x_Resolvido', 'Resolvido', 'agenda.Resolvido', 'agenda.Resolvido', 16, -1, FALSE, 'agenda.Resolvido', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->Resolvido->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Resolvido'] = &$this->Resolvido;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "agenda";
	}
	...
	function getSqlSelect() { // Select
		return ($this->_SqlSelect <> "") ? $this->_SqlSelect : "SELECT agenda.Prioridade, agenda.Data, agenda.Horario, agenda.id, agenda.Assunto, agenda.Tarefa, agenda.Resolvido FROM " . $this->getSqlFrom();
	}
	...
	function getSqlWhere() { // Where
		$sWhere = ($this->_SqlWhere <> "") ? $this->_SqlWhere : "agenda.Resolvido = 4";
		$this->TableFilter = "";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("agenda_mortaview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("agenda_mortaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "agenda_mortaadd.php?" . $this->UrlParm($parm);
		else
			return "agenda_mortaadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("agenda_mortaedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("agenda_mortaadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("agenda_mortadelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->Tarefa->Exportable) $Doc->ExportCaption($this->Tarefa);
					if ($this->Resolvido->Exportable) $Doc->ExportCaption($this->Resolvido);
				} else {
					if ($this->Prioridade->Exportable) $Doc->ExportCaption($this->Prioridade);
					if ($this->Data->Exportable) $Doc->ExportCaption($this->Data);
					if ($this->Horario->Exportable) $Doc->ExportCaption($this->Horario);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->Tarefa->Exportable) $Doc->ExportField($this->Tarefa);
						if ($this->Resolvido->Exportable) $Doc->ExportField($this->Resolvido);
					} else {
						if ($this->Prioridade->Exportable) $Doc->ExportField($this->Prioridade);
						if ($this->Data->Exportable) $Doc->ExportField($this->Data);
						if ($this->Horario->Exportable) $Doc->ExportField($this->Horario);
