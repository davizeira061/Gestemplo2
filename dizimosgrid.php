...
// Create page object
if (!isset($dizimos_grid)) $dizimos_grid = new cdizimos_grid();
...
fdizimosgrid.EmptyRow = function(infix) {
	var fobj = this.Form;
	if (ew_ValueChanged(fobj, infix, "Descricao", false)) return false;
	if (ew_ValueChanged(fobj, infix, "Receitas", false)) return false;
	if (ew_ValueChanged(fobj, infix, "FormaPagto", false)) return false;
	if (ew_ValueChanged(fobj, infix, "Dt_Lancamento", false)) return false;
	if (ew_ValueChanged(fobj, infix, "Vencimento", false)) return false;
	return true;
}
...
if (@$emptywrk) $dizimos->FormaPagto->OldValue = "";
?>
</select>
<script type="text/javascript">
...
<?php
	...
	if ($dizimos->CurrentAction <> "gridadd" || $dizimos->CurrentMode == "copy")
		if (!$dizimos_grid->Recordset->EOF) $dizimos_grid->Recordset->MoveNext();
}
?>
<?php
