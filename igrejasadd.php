...
$igrejas_add = NULL; // Initialize page object first

class cigrejas_add extends cigrejas {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["Id_igreja"] != "") {
				...
				$this->setKey("Id_igreja", $this->Id_igreja->CurrentValue); // Set up key
			} else {
				$this->setKey("Id_igreja", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "igrejasview.php")
	...
	function LoadDefaultValues() {
		$this->Igreja->CurrentValue = NULL;
		$this->Igreja->OldValue = $this->Igreja->CurrentValue;
		...
		$this->Telefone1->CurrentValue = NULL;
		$this->Telefone1->OldValue = $this->Telefone1->CurrentValue;
		$this->Telefone2->CurrentValue = NULL;
		$this->Telefone2->OldValue = $this->Telefone2->CurrentValue;
		$this->Fax->CurrentValue = NULL;
		$this->Fax->OldValue = $this->Fax->CurrentValue;
		$this->DirigenteResponsavel->CurrentValue = NULL;
		$this->DirigenteResponsavel->OldValue = $this->DirigenteResponsavel->CurrentValue;
		$this->_Email->CurrentValue = NULL;
		$this->_Email->OldValue = $this->_Email->CurrentValue;
		$this->Site_Igreja->CurrentValue = NULL;
		$this->Site_Igreja->OldValue = $this->Site_Igreja->CurrentValue;
		$this->Email_da_igreja->CurrentValue = NULL;
		$this->Email_da_igreja->OldValue = $this->Email_da_igreja->CurrentValue;
		$this->Modelo->CurrentValue = NULL;
		$this->Modelo->OldValue = $this->Modelo->CurrentValue;
		$this->Data_de_Fundacao->CurrentValue = NULL;
		$this->Data_de_Fundacao->OldValue = $this->Data_de_Fundacao->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->Igreja->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(Igreja = '" . ew_AdjustSql($this->Igreja->CurrentValue) . "')";
		...
		// Igreja
		$this->Igreja->SetDbValueDef($rsnew, $this->Igreja->CurrentValue, NULL, FALSE);
		...
		// Telefone1
		$this->Telefone1->SetDbValueDef($rsnew, $this->Telefone1->CurrentValue, NULL, FALSE);

		// Telefone2
		$this->Telefone2->SetDbValueDef($rsnew, $this->Telefone2->CurrentValue, NULL, FALSE);

		// Fax
		$this->Fax->SetDbValueDef($rsnew, $this->Fax->CurrentValue, NULL, FALSE);

		// DirigenteResponsavel
		$this->DirigenteResponsavel->SetDbValueDef($rsnew, $this->DirigenteResponsavel->CurrentValue, NULL, FALSE);

		// Email
		$this->_Email->SetDbValueDef($rsnew, $this->_Email->CurrentValue, NULL, FALSE);

		// Site_Igreja
		$this->Site_Igreja->SetDbValueDef($rsnew, $this->Site_Igreja->CurrentValue, NULL, FALSE);

		// Email_da_igreja
		$this->Email_da_igreja->SetDbValueDef($rsnew, $this->Email_da_igreja->CurrentValue, NULL, FALSE);

		// Modelo
		$this->Modelo->SetDbValueDef($rsnew, $this->Modelo->CurrentValue, NULL, FALSE);

		// Data_de_Fundacao
		$this->Data_de_Fundacao->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->Data_de_Fundacao->CurrentValue, 7), NULL, FALSE);
		...
		if ($AddRow) {
			$this->Id_igreja->setDbValue($conn->Insert_ID());
			$rsnew['Id_igreja'] = $this->Id_igreja->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("membro", $DetailTblVar) && $GLOBALS["membro"]->DetailAdd) {
				$GLOBALS["membro"]->Da_Igreja->setSessionValue($this->Id_igreja->CurrentValue); // Set master key
				if (!isset($GLOBALS["membro_grid"])) $GLOBALS["membro_grid"] = new cmembro_grid(); // Get detail page object
				$AddRow = $GLOBALS["membro_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["membro"]->Da_Igreja->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($igrejas_add)) $igrejas_add = new cigrejas_add();

// Page init
$igrejas_add->Page_Init();

// Page main
$igrejas_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$igrejas_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var igrejas_add = new ew_Page("igrejas_add");
igrejas_add.PageID = "add"; // Page ID
var EW_PAGE_ID = igrejas_add.PageID; // For backward compatibility

// Form object
var figrejasadd = new ew_Form("figrejasadd");

// Validate form
figrejasadd.Validate = function() {
...
figrejasadd.Form_CustomValidate = 
...
figrejasadd.ValidateRequired = true;
<?php } else { ?>
figrejasadd.ValidateRequired = false; 
<?php } ?>

// Multi-Page properties
figrejasadd.MultiPage = new ew_MultiPage("figrejasadd",
...
figrejasadd.Lists["x_Modelo"] = {"LinkField":"x_Id","Ajax":null,"AutoFill":false,"DisplayFields":["x_Modelo","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $igrejas_add->ShowPageHeader(); ?>
<?php
$igrejas_add->ShowMessage();
?>
<form name="figrejasadd" id="figrejasadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($igrejas_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $igrejas_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="igrejas">
<input type="hidden" name="a_add" id="a_add" value="A">
<div>
<div class="tabbable" id="igrejas_add">
...
<?php } ?>
<script type="text/javascript">
figrejasadd.Lists["x_Modelo"].Options = <?php echo (is_array($igrejas->Modelo->EditValue)) ? ew_ArrayToJson($igrejas->Modelo->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("figrejasadd", "x_Data_de_Fundacao", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<?php
	if (in_array("membro", explode(",", $igrejas->getCurrentDetailTable())) && $membro->DetailAdd) {
?>
