...
class ceventos extends cTable {
	...
	function __construct() {
		...
		// Evento
		$this->Evento = new cField('eventos', 'eventos', 'x_Evento', 'Evento', '`Evento`', '`Evento`', 200, -1, FALSE, '`Evento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Evento'] = &$this->Evento;

		// Descriacao
		$this->Descriacao = new cField('eventos', 'eventos', 'x_Descriacao', 'Descriacao', '`Descriacao`', '`Descriacao`', 201, -1, FALSE, '`Descriacao`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['Descriacao'] = &$this->Descriacao;

		// DataInicio
		$this->DataInicio = new cField('eventos', 'eventos', 'x_DataInicio', 'DataInicio', '`DataInicio`', 'DATE_FORMAT(`DataInicio`, \'%d/%m/%Y\')', 133, 7, FALSE, '`DataInicio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->DataInicio->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['DataInicio'] = &$this->DataInicio;

		// DataTermino
		$this->DataTermino = new cField('eventos', 'eventos', 'x_DataTermino', 'DataTermino', '`DataTermino`', 'DATE_FORMAT(`DataTermino`, \'%d/%m/%Y\')', 133, 7, FALSE, '`DataTermino`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->DataTermino->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['DataTermino'] = &$this->DataTermino;

		// Anotacoes
		$this->Anotacoes = new cField('eventos', 'eventos', 'x_Anotacoes', 'Anotacoes', '`Anotacoes`', '`Anotacoes`', 201, -1, FALSE, '`Anotacoes`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->Evento->Exportable) $Doc->ExportCaption($this->Evento);
					if ($this->Descriacao->Exportable) $Doc->ExportCaption($this->Descriacao);
					if ($this->DataInicio->Exportable) $Doc->ExportCaption($this->DataInicio);
					if ($this->DataTermino->Exportable) $Doc->ExportCaption($this->DataTermino);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->Evento->Exportable) $Doc->ExportField($this->Evento);
						if ($this->Descriacao->Exportable) $Doc->ExportField($this->Descriacao);
						if ($this->DataInicio->Exportable) $Doc->ExportField($this->DataInicio);
						if ($this->DataTermino->Exportable) $Doc->ExportField($this->DataTermino);
